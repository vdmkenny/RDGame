...
class GameCharacter(arcade.Sprite):
    ...
    def set_moving(self, direction=None, stop=True):
    ...
    def __init__(self, character, properties={}):
        ...
        self.character_moving = bool(self.properties.get("moving", False))
        self.character_face_direction = int(self.properties.get("facing", DOWN_FACING))
        self.character_type = str(self.properties.get("type", "None"))

        if self.properties.get(self.character_type == "npc"):
        ...
        self.textures_down = load_character_textures(
            f"{MAIN_PATH}/spritemap.png", self.character, DOWN_FACING
        )
        self.textures_up = load_character_textures(
            f"{MAIN_PATH}/spritemap.png", self.character, UP_FACING
        )
        self.textures_left = load_character_textures(
            f"{MAIN_PATH}/spritemap.png", self.character, LEFT_FACING
        )
        self.textures_right = load_character_textures(
            f"{MAIN_PATH}/spritemap.png", self.character, RIGHT_FACING
        )
        ...
        # self.character_face_direction = int(self.properties.get('facing', down_facing))
        self.set_facing(int(self.properties.get("facing", DOWN_FACING)))

        if self.properties.get("moving"):
            self.set_moving(self.character_face_direction, False)
        ...
        self.action_sprite.texture = load_character_textures(
            f"{MAIN_PATH}/spritemap.png", self.character, DOWN_FACING
        )[0]
        ...
        self.action_sprite.set_hit_box(
            [
                [
                    -(CHARACTER_RESOLUTION // CHARACTER_SCALING // 2),
                    -(CHARACTER_RESOLUTION // CHARACTER_SCALING // 2),
                ],
                [
                    (CHARACTER_RESOLUTION // CHARACTER_SCALING // 2),
                ...
                [
                    (CHARACTER_RESOLUTION // CHARACTER_SCALING // 2),
                    -(CHARACTER_RESOLUTION // CHARACTER_SCALING),
                ],
                [
                    -(CHARACTER_RESOLUTION // CHARACTER_SCALING // 2),
                    -(CHARACTER_RESOLUTION // CHARACTER_SCALING),
    ...
    def check_for_collision(self, game):
        collision_list = game.activemap.map_dict.get(
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        self.switch_action = {
            DOWN_FACING: [
                self.left,
                self.bottom - (CHARACTER_RESOLUTION // CHARACTER_SCALING) - 16,
            ],
            UP_FACING: [self.left, self.top + 16],
            LEFT_FACING: [
                self.left - (CHARACTER_RESOLUTION // CHARACTER_SCALING) - 16,
                self.bottom,
            ],
            RIGHT_FACING: [self.right + 16, self.bottom],
        }
        self.action_sprite.left = self.switch_action.get(
            int(self.character_face_direction)
        )[0]
        self.action_sprite.bottom = self.switch_action.get(
            int(self.character_face_direction)
        )[1]
    ...
    def interact(self, game):
        ...
        if interactions:
            ...
            interactions[0].character_face_direction = switch_face.get(
                self.character_face_direction
            )

        interactions += self.action_sprite.collides_with_list(
            game.activemap.map_dict.get("message_layer")
        )
        if interactions:
            game.dialog.setMessage(
                name=interactions[0].properties.get("display_name", None),
